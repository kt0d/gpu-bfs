...
__device__ bool warp_cull(const int v)
{
    volatile extern __shared__ int scratch[]; //[WARPS][HASH_RANGE];
    ...
    if (v != -1)
        scratch[index] = v;
    __syncwarp();
    const int retrieved = scratch[index];//[warp_id][hash];
    if (retrieved == v)
    {
	    // Vie to be the only thread in warp inspecting vertex v
        scratch[index] = threadIdx.x;
    }
    __syncwarp();
    if (retrieved == v && scratch[index] != threadIdx.x)
    {
...
__device__ int2 block_prefix_sum(const int val)
{
    ...
    extern __shared__ int sums[];//[WARPS];
    int value = val;
...
#pragma unroll
	...
	// Mask with bit set for every needed thread, since WARPS could be less than 32
...
{
    volatile extern  __shared__ int comm[];//[3];
    ...
    while(__syncthreads_or(r_end-r))
    {
        ...
        while((total - block_progress) > 0)
        {
            ...
            volatile extern __shared__ int base_offset[];//[1]
...
__device__ void fine_grained_gather(const int* const column_index, int* const distance,cudaSurfaceObject_t bitmask_surf, const int iteration, int * const out_queue, int* const out_queue_count,int r, int r_end)
{
    ...
    extern __shared__ int block_comm[];//[BLOCK_SIZE];
    int cta_progress = 0;
    int remain;
    ...
    while ((remain = total - cta_progress) > 0)
    {
        ...
        {
		// Write indices of vertices to be inspected in shared array
            block_comm[rsv_rank - cta_progress] = r;
            rsv_rank++;
            r++;
        }
        __syncthreads();
        ...
        {
            neighbor = column_index[block_comm[threadIdx.x]];
            // Look up status
            is_valid = status_lookup(distance,bitmask_surf, neighbor);
            if(is_valid)
            {
        ...
        __syncthreads();
	// Perform prefix sum to get offset in out queue and total number of vertices to be saved in out queue
        const int2 queue_offset = block_prefix_sum(is_valid?1:0);
...
__global__ void expand_contract_bfs(const int n, const int* row_offset, const int* column_index, int* distance, const int iteration,const int* in_queue,const int* in_queue_count, int* out_queue, int* out_queue_count, cudaSurfaceObject_t bitmask_surf)
{
    ...
    //if(tid >= *in_queue_count) return; // you can't do this because of all subsequent __syncthreads and __syncwarp calls in device functions

    // Get vertex from the queue
    const int v = tid < (*in_queue_count)? in_queue[tid]:-1;

    // Local warp-culling
    bool is_duplicate = warp_cull(v);
    ...
    // If adjacency list is longer or equal to BLOCK_SIZE, use block-wide coarse grained gathering. Otherwise use fine-grained gathering.
...
__global__ void contract_expand_bfs(const int n, const int* row_offset, const int* column_index, int* distance, const int iteration,const int* in_queue,const int* in_queue_count, int* out_queue, int* out_queue_count)
{
    const int tid = blockIdx.x*blockDim.x + threadIdx.x;
    // Get vertex from the queue
    int v = tid < (*in_queue_count)? in_queue[tid]:-1;
	if(!status_lookup(distance,0,v))
		v = -1;
    // Local warp-culling
    bool is_duplicate = warp_cull(v);
    if(v == -1) is_duplicate= true;
