...
__global__ void init_distance(const int n, int* distance,const int start)
{
...
__global__ void quadratic_bfs(const int n, const int* row_offset, const int* column_index, int* distance, const int iteration, bool* done)
{
    // Calculate corresponding vertex
    int id = blockIdx.x*blockDim.x + threadIdx.x;
...
__global__ void linear_bfs(const int n, const int* row_offset, const int* column_index, int* distance, const int iteration,const int* in_queue,const int* in_queue_count, int* out_queue, int* out_queue_count)
{
...
bfs::result run_linear_bfs(csr::matrix graph, int starting_vertex)
{
    // Allocate device memory for graph and distance vector
    int *d_distance, *d_row_offset, *d_column_index;
    checkCudaErrors(cudaMalloc((void**)&d_distance,graph.n * sizeof(int)));
    ...
    // Allocate queues and queue counters
    int *d_in_queue_count, *d_out_queue_count;
    int *h_in_queue_count, *h_out_queue_count;
    int *d_in_queue, *d_out_queue;

    checkCudaErrors(cudaMalloc((void**)&d_in_queue,graph.n * sizeof(int)));
    checkCudaErrors(cudaMalloc((void**)&d_out_queue,graph.n * sizeof(int)));
    ...
    int num_of_blocks = div_up(graph.n,block_dim);
    init_distance<<<num_of_blocks,block_dim>>>(graph.n, d_distance,starting_vertex);
    ...
    *h_out_queue_count=0;
    ...
    while(*h_in_queue_count > 0)
    {
        ...
        *h_out_queue_count = 0;
        ...
        // Calculate number of blocks
        num_of_blocks = div_up(*h_in_queue_count,block_dim);

        // Run kernel
        linear_bfs<<<num_of_blocks,block_dim>>>(graph.n,d_row_offset,d_column_index,d_distance,iteration, d_in_queue,d_in_queue_count, d_out_queue, d_out_queue_count);
...
bfs::result run_quadratic_bfs(csr::matrix graph, int starting_vertex)
{
    // Allocate device memory for graph and distance vector
    int *d_distance, *d_row_offset, *d_column_index;
    checkCudaErrors(cudaMalloc((void**)&d_distance,graph.n * sizeof(int)));
    ...
    {
        *h_done=true;
        quadratic_bfs<<<num_of_blocks,block_dim>>>(graph.n,d_row_offset,d_column_index,d_distance,iteration, d_done);
        checkCudaErrors(cudaDeviceSynchronize());
        iteration++;
    } while(!(*h_done));
    ...
    *h_out_queue_count=0;
    ...
    while(*h_in_queue_count > 0)
    {
        ...
        *h_out_queue_count = 0;
        ...
        // Run kernel
        expand_contract_bfs<<<num_of_blocks,block_dim>>>(graph.n,d_row_offset,d_column_index,d_distance,iteration, d_in_queue,d_in_queue_count, d_out_queue, d_out_queue_count);
        checkCudaErrors(cudaDeviceSynchronize());
