	...
	*h_out_queue_count=0;
	...
	while(*h_in_queue_count > 0)
	{
		...
		*h_out_queue_count = 0;
		...
		// Run kernel
		expand_contract_bfs<<<num_of_blocks,BLOCK_SIZE>>>(graph.n,d_row_offset,d_column_index,d_distance,iteration, d_in_queue,d_in_queue_count, d_out_queue, d_out_queue_count,bitmask_surf);
		checkCudaErrors(cudaDeviceSynchronize());
